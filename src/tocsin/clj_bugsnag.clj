;; Originally copied from https://github.com/wunderlist/clj-bugsnag
...
(defn all-exceptions [project-ns ex]
  (loop [ex    ex
         accum []]
    (if ex
      (recur (:cause ex)
             (conj accum {:errorClass (.getName (:class ex))
                          :message    (:message ex)
  ...
  (let [ex (parse-exception exception)
    ...
    {:apiKey (:api-key options (env :bugsnag-key))
     ...
     :events [{:payloadVersion "2"
               :exceptions (all-exceptions project-ns ex)
